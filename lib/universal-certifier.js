...
const UC = function () {
  ...
  /**
   ...
   @return json
   */
  uc.certify = function (pictures) {
    const next = typeof (arguments[arguments.length - 1]) === 'function' ? arguments[arguments.length - 1] : null;
    if (typeof pictures !== 'object') throw new UniversalCertifierError('Wrong parameters, check documentation');
    if (!pictures || !pictures.length || pictures.length === 0) throw new UniversalCertifierError('Wrong parameters, missing payload');
    pictures.forEach((picture) => {
      if (!picture.name || !picture.link) throw new UniversalCertifierError('Wrong parameters, check documentation');
    });
